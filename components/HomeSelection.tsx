...
const HomeSelection: React.FC<HomeSelectionProps> = (props) => {
  const { isMobileLayout, appTitle } = props;
  ...
  return (
    <div className="relative text-center w-full animate-fade-in">
      ...
      <div className={`grid ${isMobileLayout ? 'grid-cols-1 gap-3' : 'md:grid-cols-2 lg:grid-cols-4 gap-6'}`}>
        {mainCategories.map((category, index) => {
          const colors = colorClasses[category.color as keyof typeof colorClasses];
          const action = props[category.action as keyof Omit<HomeSelectionProps, 'isMobileLayout' | 'appTitle'>];
          ...
          return (
            <button
                ...
                className={`group animate-fade-in flex ${isMobileLayout ? 'flex-row items-center justify-start text-left px-6 py-3' : 'flex-col items-center justify-center p-6'} rounded-xl shadow-lg transition-all duration-300 ease-in-out transform hover:-translate-y-2 hover:shadow-2xl ${colors.bgColor} border-2 border-transparent ${colors.hoverBorder} focus:outline-none focus:ring-4 ${colors.focusRing}`}
            >
